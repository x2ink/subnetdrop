...
- `WebSocketSession.send` 成功只表示帧进入本地 outgoing channel，不能作为对端已接收进度；Ktor 3.5 默认无界队列
  还会让发送端提前排入大量 ByteArray，造成虚假 100%、内存占用和 GC 压力。
- 双端进度应以接收端确认落盘的累计字节为唯一事实。使用多分块窗口级签名确认可以限制漂移和内存，又不会像逐块
  ACK 那样给每个 512 KiB 都增加一次网络往返。
