...
- Compose 修饰符顺序会同时影响测量、命中范围和 indication 绘制。若先 `clickable` 再 `padding`，外边距也会成为
  可点击区域，hover/按压底色会按整行矩形显示，而不是按可见卡片显示。
- 复用卡片应把外层布局修饰符与交互修饰符分开：先应用外边距，再按卡片真实 shape 裁剪，最后挂载点击、长按和
  右键手势。这样不同入口复用同一内容时不会各自形成不同的交互边界。

## 弹出菜单必须锚定可见消息卡片

- `DropdownMenu` 使用其父布局的边界定位。文件/媒体卡片为了左右对齐通常包在 `fillMaxWidth` 容器里；如果菜单
  与卡片只共享这个整行容器，发送方菜单会从聊天区域左侧开始布局，看起来与右侧消息脱离。
- 全宽容器只负责方向对齐；实际卡片与菜单应再放入同一个内容尺寸 `Box`。这样锚点边界就是气泡本身，同时保留
  Material 弹层的窗口边缘避让能力，普通文件、图片和视频必须使用同一规则。

## 桌面文件剪贴板不是路径字符串

- 桌面“复制文件”必须写入系统文件列表格式（JVM/AWT 的 `javaFileListFlavor`）；复制文件名或路径文本只能粘贴为
  字符，Finder、Explorer 和其他应用不会把它识别成可操作的文件对象。
- 文件粘贴不能复用文字剪贴板读取。输入框只在 Ctrl/Cmd+V 且剪贴板实际包含文件列表时拦截快捷键，普通文本继续
  交给文本框；文件仍需复用选择器/拖放的数量、类型和大小校验。
- 从剪贴板取得文件不等于用户授权发送。先形成只存在于 UI 的待确认列表，确认后才调用正式发送入口；取消时不能
  创建文件消息、网络任务或数据库记录。
- 手势识别和视觉 indication 是两件事。消息项可以保留点击、双击、长按和右键识别，同时使用独立 interaction
  source 和空 indication 移除 hover、按压与涟漪反馈。
