...
- [x] 确认发送方文字、普通文件和媒体文件消息的菜单锚点边界。
- [x] 将文件消息操作菜单锚定到实际消息卡片，而不是占满聊天宽度的时间线行。
- [x] 更新交互规格与经验记录，并完成共享 JVM、桌面编译和 Android shared 编译验证。

### 审查

- 根因是文件操作菜单与 `fillMaxWidth` 时间线容器共享锚点；发送卡片虽靠右，菜单仍从聊天区域左边界计算位置。
- 普通文件卡片现在与菜单共享内容尺寸 `Box`；图片和视频卡片也在定宽卡片层内承载菜单，左右消息各自跟随卡片。
- JVM 测试、桌面编译和 Android shared 编译通过；桌面交互复测需要先退出当前占用监听端口的旧实例。

## 当前计划：无反馈消息手势与桌面文件剪贴板

- [x] 消息项保留双击、长按、右键和多选点击语义，但移除 hover、按压与涟漪视觉反馈。
- [x] 文件消息“复制”在桌面写入系统文件列表剪贴板，Android 保留安全的 URI/路径剪贴板降级。
- [x] 桌面聊天输入框识别 Ctrl/Cmd+V 文件列表，复用现有文件预检，并在用户确认后才发送。
- [x] 补齐中英日确认与错误文案、平台剪贴板测试、规格、经验和验证记录。
- [x] 运行共享 JVM、桌面和 Android shared 验证，不安装 Android 应用。

### 审查

- 消息内容的普通点击、双击、长按、右键和失败重试均使用无 indication 的交互源，不再显示 hover/按压反馈。
- 桌面文件复制使用 AWT `javaFileListFlavor`，可被 Finder、Explorer 及支持系统文件剪贴板的程序识别。
- 文件粘贴仅在输入框聚焦且剪贴板含文件列表时拦截 Ctrl/Cmd+V；文件预检完成后必须确认，取消不产生传输。
- 共享 JVM 测试、桌面和 Android shared 编译通过；Android Debug APK 构建结果见验证记录，未安装应用。

## 当前计划：媒体缩略图与独立比例

- [x] 图片消息保持 4:3，但使用明确的目标尺寸请求生成预览缩略图，外部打开仍使用原始文件。
- [x] 视频消息从 4:3 调整为 16:9，传输态、完成态和失败态保持同一比例。
- [x] 补充比例纯逻辑测试并更新媒体规格、README 与验证记录。
- [x] 运行共享 JVM 测试、桌面和 Android shared 编译，不安装 Android 应用。

### 审查

- 图片预览通过 Coil 请求 720×540 的精确目标尺寸，缓存与绘制使用缩略图结果；点击打开仍指向原始本地文件。
- 图片卡保持 4:3，视频卡独立使用 16:9；两者继续保持无气泡尖角的圆角矩形。
- 共享 JVM 测试、桌面编译和 Android shared 编译通过，未安装 Android 应用。

## 当前计划：统一文字与文件消息操作

- [x] 将消息操作入口统一为 Android/桌面长按、桌面双击和右键，文字与文件共用菜单状态模型。
- [x] 为终态文件消息增加复制文件名、转发、仅删除本地记录和混合多选；活动传输保留取消入口并限制破坏性操作。
- [x] 让 Android `content://` 文件在转发前复制到发送缓存，桌面路径直接复用，并让删除同步移除终态实时传输项。
- [x] 将转发目标从居中 Dialog 改为 Material 3 底部弹层，抽取并复用首页附近设备卡片 UI。
- [x] 补充中英日文案、领域/数据/UI 测试、规格、经验和验证记录，不安装 Android 应用。

### 审查

- 文字与文件统一支持双击、长按和桌面右键；文件保留单击系统打开，活动传输不允许删除或转发。
- 转发选择改为 Material 3 底部弹层，并直接复用首页 `PeerListItem`，只展示在线且已信任设备。
- 文件删除同时更新 SQLDelight 和实时终态列表，且不删除实体文件；Android 提供方 URI 会先复制到发送缓存。
- JVM/桌面测试及 Android Debug APK 构建通过，未安装 Android 应用；验证结果见
  `docs/tasks/verification/2026-09-09-message-file-actions.md`。

## 当前计划：图片与视频消息预览

- [x] 定义图片、视频和普通文件的稳定分类规则，以及完成、传输中、失败和已失效状态的展示边界。
- [x] 使用 Coil 3 + FileKit 加载跨平台本地图片，Android 使用 Coil Video、桌面使用 JCodec 提取视频首帧。
- [x] 将图片和视频消息改为固定 4:3 媒体卡片，重做传输中进度遮罩，同时保留系统默认应用打开行为。
- [x] 补充媒体分类与预览状态测试、产品规格、架构入口、经验和验证记录。
- [x] 最多进行 3 轮编译/测试修正，运行共享 JVM、桌面和 Android shared 验证，不安装 Android 应用。

### 审查

- 媒体类型优先使用 MIME，只有缺失或通用二进制 MIME 才回退扩展名，普通文件 UI 和传输协议不变。
- 完成且本地文件存在才加载预览；发送方传输中也不会提前显示源文件，从视觉语义上区分“可读取”和“已送达”。
- Coil 3.5.0 与 FileKit 0.15.0 对齐并保持 Compose 1.11.1；华为云国内镜像补齐了阿里云缺失的 FileKit Coil JAR。
- Android Debug APK 仅完成构建、未安装；真实 Android `content://` 预览和 Windows 视频首帧仍需目标设备视觉验收。
- 自动验证结果见 `docs/tasks/verification/2026-09-09-media-message-preview.md`。
