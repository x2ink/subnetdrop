...
4. 两边核对并确认相同的六位安全码。
5. 进入一对一会话发送消息；使用输入框旁的回形针按钮选择文件。
6. 对端默认自动接收；如需逐次决定，可在“设置”开启“接收文件前确认”。
7. 可在“设置”修改文件保存位置；Android 默认保存到系统公共 `Download/SubnetDrop`，桌面默认保存到
   `~/Downloads/SubnetDrop`，同名文件不会被覆盖。
8. 接收完成后点击文件消息，使用操作系统默认应用打开。
