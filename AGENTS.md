...
- 产品是以高速文件直传为核心、兼具加密一对一即时聊天的开源局域网工具，支持 Android 11+、macOS 和 Windows。
...
- 聊天正文必须保持端到端加密。文件内容为高速明文二进制流，仅允许经过确认的受信设备发送。
- 身份响应、文件控制帧、ACK 和已读回执必须验证签名与发送方/接收方。加密载荷必须把协议版本、
  帧类型、发送方和接收方绑定到 associated data，防止跨帧重放。
- 所有外部输入都要限制长度、数量和格式；协议错误必须显式返回，不能静默降级成明文。
- 消息接收保持幂等；ACK、已读回执和文件块只能更新属于对应受信对端的数据。
- 文件传输必须先提议再按接收策略确认；控制面使用签名 WebSocket，文件正文使用带 `Content-Length` 的
  HTTP/1.1 流式请求。上传需要短时一次性 Token 和绑定双方身份、传输 ID、Token、长度的 Ed25519 请求签名；
  用户可配置 1–1024 GiB 单文件上限，默认 10 GiB，协议绝对上限为 1 TiB，并保持最终 SHA-256 校验。
...
- 底部主导航只保留“附近设备 / 设置”；聊天从受信设备项进入。桌面宽屏使用双栏，紧凑窗口使用单栏返回导航。
